...
<template>
  <div class="grid md:grid-cols-2 xl:flex items-center space-x-6">
    <div class="flex items-center space-x-2 border">
      ...
      <label v-if="filters.hue.enabled" class="text-sm fadeIn"
    ...
    <div class="flex items-center space-x-2 border">
      <label>
        <input v-model="filters.lightness.enabled" type="checkbox" />
        <strong>Lightness</strong>
      </label>
      <input
          v-model.number="filters.lightness.value"
          :disabled="!filters.lightness.enabled"
          :max="filters.lightness.maxValue"
          :min="filters.lightness.minValue"
          :step="0.1"
          class="w-20"
          type="range"
      />
